...
static char* sm4_cbc_encrypt(cipher_interface_t* self, const char* text)
{
    ...
    const sm4_cbc_android_old_data_t* d = self->private_data;
...
cipher_interface_t* create_sm4_cbc_android_old_cipher(const uint8_t* key, const uint8_t* iv)
{
    if(!key || !iv) return NULL;
    cipher_interface_t* ci = s_malloc(sizeof(cipher_interface_t));
    sm4_cbc_android_old_data_t* d = s_malloc(sizeof(sm4_cbc_android_old_data_t));
